...
def get_word_probability(word, tag, word_dict):   
    ...
    if(dict_tagged != None):
        value =  dict_tagged.get(word,0) * 1.0 /sum
        ...
        if(word[-1] == 's' and (tag == 'NNS' or tag == 'VBZ')):
           value = value + 0.00001
        if(word[-2:] == 'ed' and tag == 'VBN'):
            value = value + 0.001
        if(word[-2:] == 'ly' and tag == 'RB'):
            value = value + 0.001
        if((word[-4:] == 'able' or word[-2:] == 'al' or ('-' in word)) and tag == 'JJ'):
            value = value + 0.0001
        if(word[-3:] == 'ion' and tag == 'NN'):
            value = value + 0.0001
        if(word[-3:] == 'ing' and tag == 'VBG'):
            value = value + 0.0001
        return value
    else:
        return 0.0
...
def main():
    ...
    test = "D:\Academics\Courses\NLP\hw4_programming\WSJ_POS_CORPUS_FOR_STUDENTS\WSJ_23.words"
    result_file = "D:\Academics\Courses\NLP\hw4_programming\WSJ_POS_CORPUS_FOR_STUDENTS\wsj_23.pos" 
    f = open(test)
